...
def bank_data2():
    ...
    # Build the system message based on the attributes in the data.
    ...
    for col in col_names[:-1]: # ignores the y value
        if col in categorical_unique_values.keys():
            ...
            list_of_attributes_with_values += f" {col} (continuous numerical value, if the user is younger than 40 then change the counterfactual age will be 50 and if the user is older than 40 than the counterfactual age will be 25)"
        else:
            list_of_attributes_with_values += f" {col} (numerical value)"
    ...
    # Set up the range of each feature
    input_bound = []
    # TODO: Finish adding the range.
    ...
    return X, Y, input_shape, nb_classes, system_message, label_encoders
