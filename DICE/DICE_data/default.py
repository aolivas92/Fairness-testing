...
from DICE_utils.config2 import default
...
def default_data2():
    ...
    for col in col_names[:-1]: # ignores the y value
        minimum = int(math.floor(df_encoded[col].min()))
        maximum = int(math.ceil(df_encoded[col].max()))
    ...
    default.params = len(col_names[:-1])
    default.input_bounds = input_bounds
    default.feature_name = col_names[:-1]
    # default.class_name =
    # default.categorical_features =
    default.system_message = system_message
    default.label_encoders = label_encoders
    default.categorical_unique_values = categorical_unique_values
